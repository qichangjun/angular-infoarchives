(function() {
  ...
  angular.module("myApp").controller("dataModuleController", [
    '$scope', '$log', '$stateParams', '$mdDialog', 'projectManageService', '$timeout', 'mdDialogService', 'dataModuleService', 'uuid', 'basicDataService', 'commonMethodSerivce', '$translate', function($scope, $log, $stateParams, $mdDialog, projectManageService, $timeout, mdDialogService, dataModuleService, uuid, basicDataService, commonMethodSerivce, $translate) {
      ...
      getVersionList = function() {
        ...
        return dataModuleService.getModuleVersionList(vm.parameter.objectId).then(function(res) {
          ...
          if (vm.versionList.length === 0) {
            vm.updateAble = false;
            $scope.nodes = {
              "nameZh": vm.projectName,
              "nameEn": vm.projectName,
              "name": vm.projectName,
              "code": uuid.v4(),
              "type": "record",
              "children": []
      ...
      listenEvent = function() {
        ...
        $scope.$on('node:updateName', function(e, d) {
          var brothersEnName, brothersName, brothersZhName, item;
          nameList = [];
          ...
          brothersZhName = (function() {
            var j, len, results;
            results = [];
            for (j = 0, len = nameList.length; j < len; j++) {
              item = nameList[j];
              results.push(item.nameZh);
            }
            return results;
          })();
          brothersEnName = (function() {
            var j, len, results;
            results = [];
            for (j = 0, len = nameList.length; j < len; j++) {
              item = nameList[j];
              results.push(item.nameEn);
            }
            return results;
          })();
          return mdDialogService.initCustomDialog('updateNodeNameController', PAHT_OF_TEMPLATE_MDDIALOG + 'updateNodeName.html?' + window.hsConfig.bust, event, {
            ...
            brothersZhName: brothersZhName,
            brothersEnName: brothersEnName,
            canRepeat: d.canRepeat,
            type: d.type
          }).then(function(res) {
            if (res && !commonMethodSerivce.includeInArr(res.nameZh, brothersZhName) && !commonMethodSerivce.includeInArr(res.nameEn, brothersEnName) && !commonMethodSerivce.includeInArr(res.name, brothersName)) {
              updateName($scope.nodes, d.code, res.canRepeat, res.nameZh, res.nameEn);
            } else {
      ...
      updateName = function(node, id, canRepeat, nameZh, nameEn) {
        var i, j, len, ref, rows;
        if (node.code === id) {
          node.nameZh = nameZh;
          node.nameEn = nameEn;
          node.canRepeat = canRepeat;
          $scope.$broadcast('update:name', id, nameZh, nameEn);
        } else if (node.children) {
          ref = node.children;
          for (i = j = 0, len = ref.length; j < len; i = ++j) {
            rows = ref[i];
            if (rows.code === id) {
              rows.nameZh = nameZh;
              rows.nameEn = nameEn;
              rows.canRepeat = canRepeat;
              $scope.$broadcast('update:name', id, nameZh, nameEn);
              return;
            } else {
              updateName(rows, id, canRepeat, nameZh, nameEn);
            }
          }
        }
      };
      addContainer = function(node, id, type, name) {
        ...
        if (node.code === id) {
          code = uuid.v4();
          node.children.push({
            "nameZh": $translate.instant("MODULES_CONTAINER_TYPE_" + type.toUpperCase()) + name,
            "nameEn": $translate.instant("MODULES_CONTAINER_TYPE_" + type.toUpperCase()) + name,
    ...
    '$scope', '$log', '$stateParams', '$mdDialog', 'commonMethodSerivce', 'brothersEnName', 'brothersZhName', 'mdToastService', '$translate', 'canRepeat', 'nameZh', 'nameEn', 'type', 'brothersName', 'name', function($scope, $log, $stateParams, $mdDialog, commonMethodSerivce, brothersEnName, brothersZhName, mdToastService, $translate, canRepeat, nameZh, nameEn, type, brothersName, name) {
      var cancel, init, update, vm;
      vm = this;
      vm.type = type;
      vm.canRepeat = canRepeat;
      init = function() {
        var i, j, k, l, len, len1, len2, rows;
        for (i = j = 0, len = brothersEnName.length; j < len; i = ++j) {
          rows = brothersEnName[i];
          if (rows === nameEn) {
            brothersEnName.splice(i, 1);
            break;
          }
        }
        for (i = k = 0, len1 = brothersZhName.length; k < len1; i = ++k) {
          rows = brothersZhName[i];
          if (rows === nameZh) {
            brothersZhName.splice(i, 1);
            break;
          }
        }
        for (i = l = 0, len2 = brothersName.length; l < len2; i = ++l) {
          rows = brothersName[i];
          if (rows === name) {
      ...
      update = function() {
        if (!vm.nameZh || !vm.nameEn || !vm.name) {
          mdToastService.showToast($translate.instant('MODULES_PROJECTMANAGE_NODE_NAME_CAN_NOT_BE_REPEATED'));
          return;
        }
        if (!commonMethodSerivce.includeInArr(vm.nameZh, brothersZhName) && !commonMethodSerivce.includeInArr(vm.nameEn, brothersEnName) && !commonMethodSerivce.includeInArr(vm.name, brothersName)) {
          return $mdDialog.hide({
            nameZh: vm.nameZh,
            nameEn: vm.nameEn,
            name: vm.name,
